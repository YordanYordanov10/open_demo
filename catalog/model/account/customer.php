...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', eik = '" . $this->db->escape(isset($data['eik']) ? $data['eik'] : '') . "', company = '" . $this->db->escape(isset($data['company']) ? $data['company'] : '') . "', manager = '" . $this->db->escape(isset($data['manager']) ? $data['manager'] : '') . "', address = '" . $this->db->escape(isset($data['address']) ? $data['address'] : '') . "', city = '" . $this->db->escape(isset($data['city']) ? $data['city'] : '') . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . (int)!$customer_group_info['approval'] . "', date_added = NOW()");
	...
	public function editCustomer($customer_id, $data) {
		$this->db->query("UPDATE " . DB_PREFIX . "customer SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', eik = '" . $this->db->escape(isset($data['eik']) ? $data['eik'] : '') . "', company = '" . $this->db->escape(isset($data['company']) ? $data['company'] : '') . "', manager = '" . $this->db->escape(isset($data['manager']) ? $data['manager'] : '') . "', address = '" . $this->db->escape(isset($data['address']) ? $data['address'] : '') . "', city = '" . $this->db->escape(isset($data['city']) ? $data['city'] : '') . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "' WHERE customer_id = '" . (int)$customer_id . "'");
	}
	...
	public function getTotalCustomersByEik($eik) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE eik = '" . $this->db->escape($eik) . "'");
	...
	public function getCompanies(){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "company");
