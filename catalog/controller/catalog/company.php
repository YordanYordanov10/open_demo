...
class ControllerCatalogCompany extends Controller {
    ...
    public function index() {
        $this->document->setTitle('Sample Company API');
        ...
        $data['breadcrumbs'][] = [
            'text' => 'Sample Company API',
            'href' => $this->url->link('catalog/company')
        ];
        ...
        $this->response->setOutput($this->load->view('catalog/company', $data));
    }
    ...
    public function eik() {
        ...
        if (isset($this->session->data['ip_rate'][$ip]) && (time() - $this->session->data['ip_rate'][$ip] < 2)) {
        ...
        if (isset($this->session->data['eik_page_load'])) {
            if ((time() - $this->session->data['eik_page_load']) < 2) {
        ...
        // Normalize: strip BG prefix, spaces, dashes — then validate 9 digits
        $eik = $this->request->get['eik'] ?? '';
        $eik = preg_replace('/^BG/i', '', trim($eik));
    ...
    public function submit() {
        ...
        if (!empty($this->request->post['subject_line'])) {
            $json['success'] = 'Заявката е получена!';
            $this->response->addHeader('Content-Type: application/json');
            $this->response->setOutput(json_encode($json));
            return;
        }
        ...
        // Normalize EIK server-side (handles BG prefix, spaces, dashes)
        $raw_eik = $this->request->post['eik'] ?? '';
        $eik     = preg_replace('/^BG/i', '', trim($raw_eik));
        $eik     = preg_replace('/[\s\-]/', '', $eik);
        ...
        if (!isset($json['errors']) && !isset($json['error'])) {
            ...
            if (empty($company)) {
                $json['errors']['company'] = 'Името на фирмата е задължително поле.';
            }
